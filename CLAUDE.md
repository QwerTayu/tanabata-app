...
- **各種上限は全てソフト制限・クライアント側・localStorage管理**であり、Firestoreルールやサーバーでは強制しない: 1ブラウザ・1ルームにつき短冊は最大5個、1ブラウザ・1短冊につきいいねは最大3回。ブラウザ初回アクセス時に生成する`clientId`(UUID)をlocalStorageに保存し、作成した短冊の`authorClientId`フィールドとしても使う。
